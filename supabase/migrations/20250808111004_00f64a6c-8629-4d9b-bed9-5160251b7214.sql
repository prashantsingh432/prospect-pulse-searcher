...
-- Attach triggers
create trigger trg_master_prospects_updated
...
create trigger trg_project_users_updated
before update on public.project_users
for each row execute procedure public.set_updated_at();

create trigger trg_project_prospects_updated
before update on public.project_prospects
for each row execute procedure public.set_updated_at();

create trigger trg_enrichment_jobs_updated
...
-- Policies
-- master_prospects
create policy if not exists "RTNE - Authenticated can read master prospects"
  on public.master_prospects for select
  using (auth.uid() is not null);

create policy if not exists "RTNE - Creators/Admin can insert master prospects"
  on public.master_prospects for insert
  with check (auth.uid() = created_by or get_current_user_role() = 'admin');

create policy if not exists "RTNE - Admins/Project owners can update master prospects"
...
-- projects
create policy if not exists "RTNE - Members/Admin can read projects"
...
create policy if not exists "RTNE - Users can create their own projects"
  on public.projects for insert
  with check (owner_id = auth.uid());

create policy if not exists "RTNE - Owners/Admin can update projects"
  on public.projects for update
  using (owner_id = auth.uid() or get_current_user_role() = 'admin');

-- project_users
create policy if not exists "RTNE - Users/Admin can read their project memberships"
  on public.project_users for select
  using (user_id = auth.uid() or get_current_user_role() = 'admin');

create policy if not exists "RTNE - Owners/Admin can manage project memberships"
...
create policy if not exists "RTNE - Owners/Admin can update project memberships"
...
create policy if not exists "RTNE - Owners/Admin can delete project memberships"
...
-- project_prospects
create policy if not exists "RTNE - Members/Admin can read project prospects"
...
create policy if not exists "RTNE - Members can insert project prospects they add"
...
create policy if not exists "RTNE - Admins can update project prospects"
  on public.project_prospects for update
  using (get_current_user_role() = 'admin');

create policy if not exists "RTNE - Owners/Admin can delete project prospects"
...
-- credits_log
create policy if not exists "RTNE - Users/Admin can read their credits logs"
  on public.credits_log for select
  using (user_id = auth.uid() or get_current_user_role() = 'admin');

create policy if not exists "RTNE - Users/Admin can insert credits logs"
  on public.credits_log for insert
  with check (user_id = auth.uid() or get_current_user_role() = 'admin');

-- enrichment_jobs
create policy if not exists "RTNE - Members/Admin can read enrichment jobs"
...
create policy if not exists "RTNE - Members/Admin can create enrichment jobs"
...
create policy if not exists "RTNE - Admins can update enrichment jobs"
  on public.enrichment_jobs for update
  using (get_current_user_role() = 'admin');

-- notifications
create policy if not exists "RTNE - Users/Admin can read their notifications"
  on public.notifications for select
  using (user_id = auth.uid() or get_current_user_role() = 'admin');

create policy if not exists "RTNE - Users/Admin can insert notifications"
  on public.notifications for insert
  with check (user_id = auth.uid() or get_current_user_role() = 'admin');

create policy if not exists "RTNE - Users/Admin can update their notifications"
  on public.notifications for update
  using (user_id = auth.uid() or get_current_user_role() = 'admin');

-- audit_logs
create policy if not exists "RTNE - Users/Admin can read their audit logs"
  on public.audit_logs for select
  using (user_id = auth.uid() or get_current_user_role() = 'admin');

create policy if not exists "RTNE - Users/Admin can insert audit logs"

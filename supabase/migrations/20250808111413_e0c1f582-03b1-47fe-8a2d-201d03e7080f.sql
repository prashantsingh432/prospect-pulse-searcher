-- RTNE core schema (retry - fix trigger creation)
-- Create tables
...
-- Attach triggers (drop then create to avoid IF NOT EXISTS)
drop trigger if exists trg_master_prospects_updated on public.master_prospects;
...
drop trigger if exists trg_project_users_updated on public.project_users;
create trigger trg_project_users_updated
before update on public.project_users
for each row execute procedure public.set_updated_at();

drop trigger if exists trg_project_prospects_updated on public.project_prospects;
create trigger trg_project_prospects_updated
before update on public.project_prospects
for each row execute procedure public.set_updated_at();

drop trigger if exists trg_enrichment_jobs_updated on public.enrichment_jobs;
